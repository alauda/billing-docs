...
:::warning
Operation statistics is a measurement feature provided by the platform, which will be replaced by new metering and billing functionality in the future. Please use this feature cautiously.
:::
...
It supports viewing project quotas for the current month (default), the last month, and the last 3 months (current month and the previous two complete months), and provides usage statistics for container groups, container group quotas, namespace quotas, and total CPU/memory usage.
...
1. In the left navigation bar, click **Operation Statistics** > **Operation Overview**.
   ...
   **Note**: Container group usage is calculated based on the actual usage of Pods, while container group quotas are based on the request values (request) of Pods.
...
Statistical items include: container group usage, container group quotas, project quotas, and namespace quotas. Additionally, clicking **Export Results** allows you to export the filtered statistical data as a CSV format report.

**Note**: Detailed data can only be viewed when the **Statistical Item** is `Container Group Quota` or `Container Group Usage`.
...
2. Follow the instructions in the illustration below to select the statistical time range, statistical items, and statistical granularity, then set the project filtering conditions and click the **Search** button to view the corresponding statistical report data.
   ...
   **Note**:
   ...
   - Container group usage: Calculated based on the actual usage of Pods, summing the average usage calculated every 5 minutes.

   - Container group quotas: Calculated based on the request values (request) of Pods.
   ...
   - Detailed data can be viewed when the **Statistical Item** is `Container Group Quota` or `Container Group Usage`.
...
Filtering can be conducted based on statistical items (container group usage, container group quotas), project name, project associated cluster, and namespaces under the cluster. Additionally, clicking **Export Results** allows you to export the filtered statistical data as a CSV format report.

Steps to operate:

1. In the left navigation bar, click **Operation Statistics** > **Metering Details**.

   **Or**: On the **Statistical Report** page, when the **Statistical Item** is `Container Group Quota` or `Container Group Usage`, click the **Details** button on the right side of a single query result record to enter the metering details page and view the detailed data.
...
2. Follow the instructions in the illustration below to select the statistical time range, statistical items, project, cluster, and namespace, then click the **Search** button to view the corresponding usage detail data.
   ...
   **Note**:
   ...
   - Sorting is available based on **Container Group Name** (first letter), **Total CPU Usage** (size), and **Total Memory Usage** (size).
